...
# 1 - subset with only ASD, mlu and word tokens 

new_df$Diagnosis <- as.factor(new_df$Diagnosis)

filter(new_df, Diagnosis == "ASD") %>% 
  select(Diagnosis, CHI_MLU, tokens_CHI )


# 2 
select(df, ID, ID, Age, ID, Age, tokens_CHI )
# It just chooses the variable 1 time. 
...
```{r}
# 1 Adding coloumn with mean number of words spoken during all visits 
...
# 2
 ...
 new_hej <- df %>% group_by (ID) %>% mutate(mean_new = summarise(mean(tokens_CHI)))
