...
author: "This is a submission for Anna, Andrea, Helle, Maria and Mia."
...
#removing the VISIT coloumn
df <- df %>% 
  select(-VISIT)

#merging the visit 1 values with th dataframe 
new_df <- merge(visit1, df)
...
#2. MLU below 1.5 at first visit - visit was removed from the df but if it hadn't this is our answer
#MLU15 <- new_df %>% 
...
(Andrea)
```{r}
#1 The kid who produced the most words on the 6th visit 

arrange(df, desc(tokens_CHI))
# Answer: The kid who produced the most words on the 6th visit was TC, with 1294 words produced. 

# 2: the kid who produced the least words on the 1st visit

arrange(df, tokens_CHI)

# Answer: bother Ryder and Tina produced zero words in the first visit. 
...
(Anna)
```{r}
#1 subset of kids with ASD, including columns of mlu and word tokens
ASD_sub <- new_df %>% 
  filter(Diagnosis == "ASD") %>% 
  select(MOT_MLU, CHI_MLU, tokens_MOT, tokens_CHI)

#trying to include same variable multiple times
multiple <- new_df %>% 
  select(MOT_MLU, MOT_MLU, MOT_MLU, MOT_MLU)
#nothing special happens, the column is only included once
```
...
```{r}
#1
df <- mutate(df, mean = mean(tokens_MOT + tokens_CHI) )
